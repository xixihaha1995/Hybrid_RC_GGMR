import datetime
import matplotlib.pyplot as plt, _0_tools as general_tool
import numpy as np
...
'''Load'''
results = general_tool.loadJSONFromOutputs("_results_gru")
...
cvrmse_ggmr =results['cvrmse_gru']
...
ax.plot(x_date, ggmr_predict,next(linecycler), label = "GRU", linewidth = line_width)
ax.set_ylabel('Load Power (kW)', fontsize=large_font)
ax.legend(prop={'size': large_font})
ax.set_title(f'RC CVRMSE:{cvrmse_rc:.2f}%{newline}GRU CVRMSE:{cvrmse_ggmr:.2f}%{newline}'
            , fontsize=large_font)
