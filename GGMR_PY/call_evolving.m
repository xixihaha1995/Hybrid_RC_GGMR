...
if size(y_test,2) < 1000
    time_inter = 60*60;
    for time_step = 1:size(y_test,2)
        d = datetime(x_date_start,'ConvertFrom','epochtime', 'Format','MMM-dd-yyyy HH:mm:ss');
        x_date(time_step) = d;
        x_date_start = x_date_start + time_inter;
    end
    titleheader = "Hourly sampling prediction performance";
else
    time_inter = 5 * 60;
    for time_step = 1:size(y_test,2)
        d = datetime(x_date_start,'ConvertFrom','epochtime', 'Format','MMM-dd-yyyy HH:mm:ss');
     ...
     titleheader = "5-mins sampling prediction performance";
