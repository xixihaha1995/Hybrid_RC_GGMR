...
def Evolving_LW_2(Priors, Mu, Sigma,Data_Test, SumPosterior, talk_to_rc, test_initial_time,
    ...
    rej = [];
    buf = 0;
    % C_mat = ones(length(Priors), 1); % sum
    of
    expected
    posterior
    for each component
        C_mat = SumPosterior
        ';
    % L_rate = 8e-3; % 0.001; % 0.001; % learning
    rate
    0.01
    pumax = 0.09;
    M_min = 2; % Minimum
    ...
    mixture
    components
    Beta_min = 0.03; % Minimum
    learning
    factor
    Beta_max = 0.07; % Maximum
    learning
    factor
    T_sigma = 2;
    eps = 1e-2;
    T_mrg = 1e-5; % .00000100; % Merge
    threhsold
    need
    to
    be
    ...
    sinit = 300; % initial
    determinan
    for genertaed component
        pr_init = 1e-2; % initial
        prior
        for genertaed component
            Sigma = Sigma + 0.00000002 * eye(size(Data_Test(1:L1,:), 1)); % Add
        small
        white
        noise
        to
    ...
    for current observation
        ...
        ind_upd = find(Post_pr == max(Post_pr) & max(Post_pr)
        ~ = 0 & updateFlag == 1); % Update
        the
        % best
        mach
        components
        ...
        Sigma(:,:, ind_upd(i)) = (1 - Rou) * Sigma(:,:, ind_upd(i)) + Rou * (Data_Test(1:L1, t) - Mu(:, ind_upd(i)))*(
                                                                                                                         Data_Test(
                                                                                                                             1
                                                                                                                         :L1, t) - Mu(:, ind_upd(
            i)))';
        ...
        % Sigma(:,:, ind_upd(i)) = (1 - Rou) * Sigma(:,:, ind_upd(i)) + Rou * (Data_Test(:, t) - Mu(:, ind_upd(i)))*(
                                                                                                                    Data_Test(
                                                                                                                    :, t) - Mu(:, ind_upd(
            i)))';
        Rout(1, i) = Rou;
        ...
        gaussian
        component
        elseif(size(Priors, 2) < M_max & & buf == 0 & existFlag
        ~ = 1)
        disp("Generating")
        buf = 0;
        gc(t) = 1;
        ...
        if min(abs(Skld_tot)) < T_mrg
            ...
            Sigma_m = (Priors(Max_r) * (Sigma(:,:, Max_r) + (Mu(:, Max_r) - Mu_m) * (Mu(
            :, Max_r) - Mu_m)')+Priors(Max_c)*(Sigma(:,:,Max_c)+(Mu(:,Max_c)-Mu_m)*(Mu(:,Max_c)-Mu_m)')) / Priors_m;
        ...
        % [Max_r Max_c] = find(Post_pr_mat == max_nondg); % find
        the
        index
        of
        maximu
        ...
        % LogLik_L = sum(log(Pr_Data_LVec)); % likelihood
        of
        the
        last
        GMM
        ...
        % LogLik_U = sum(log(Pr_Data_UVec)); % likelihood
        of
        the
        last
        GMM
...
[Priors, Mu, Sigma, expData] = Evolving_LW_2(Priors, Mu, Sigma,
                                             Data_Test, SumPosterior, talk_to_rc, test_initial_time,
                                             center_rc_y, scale_rc_y, u_measured, rc_warming_step, abcd, L_rate)
