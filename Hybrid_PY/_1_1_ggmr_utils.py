...
def online_ggmr_new_dominate(train_norm, test_norm,train, test,
    ...
    init_nb_com = min(4, look_back_batch_size)
    if not _hybrid:
    ...
    if std_bool:
        ...
        train_norm = scaler.transform(train.T).T
        old_center_y, old_scale_y = scaler.mean_[-1], scaler.scale_[-1]

    old_prior, old_mu, old_sigma = gauss_tools_one.online_init(train_norm, 4)
    ...
    for t_stamp in range(0, test_norm.shape[1], _predict_size):
        ...
        if t_stamp > _look_back_batch_size:
            ...
            if std_bool:
                _batch_prev = test[:, t_stamp - _look_back_batch_size: t_stamp]
                scaler = preprocessing.StandardScaler().fit(_batch_prev.T)
                _batch_prev_norm = scaler.transform(_batch_prev.T).T
                old_center_y, old_scale_y = scaler.mean_[-1], scaler.scale_[-1]

            Priors_init, Mu_init, Sigma_init = gauss_tools_one.EM_Init_Func(_batch_prev_norm, init_nb_com, False)
            ...
            old_prior, old_mu, old_sigma = gauss_tools_one.merge_new_into_old(t_stamp, _look_back_batch_size,
                                                              _batch_prev_norm, _batch_next_norm,
                                                              max_nbStates,
        ...
        if std_bool:
            _batch = test[:, t_stamp: t_stamp + _predict_size]
            _batch = _batch * old_scale_y + old_center_y


        this_exp_y_norm, dummy_Gaus_weights = gauss_tools_one.GMR_Func(old_prior, old_mu, old_sigma ,
                                                  _batch[:in_out_split,:], in_out_split)
        if std_bool:
            this_exp_y_norm = this_exp_y_norm * old_scale_y + old_center_y
