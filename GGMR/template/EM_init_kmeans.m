...
%[Data_id, Centers] = kmeans(Data', nbStates); 
 minc = min(Data');
 maxc = max(Data');
 %nbStates = size(Data',1);
 initialcenters = repmat(minc, nbStates, 1) + bsxfun(@times, (0:nbStates-1).', (maxc - minc) ./ (nbStates-1));
 [Data_id, Centers] = kmeans(Data', nbStates, 'dist', 'sqEuclidean', 'start', initialcenters);
Mu = Centers';
for i=1:nbStates
